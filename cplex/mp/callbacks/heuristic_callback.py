...
class RoundDown(ModelCallbackMixin, HeuristicCallback):
    ...
    def __call__(self):
        ...
        if var_indices:
            ...
            dvars = [self.model.get_var_by_index(v) for v in var_indices]
...
def try_heuristic_cb_on_file(filename):
    ...
    mdl = ModelReader().read_model(filename)
    if mdl:
        try_heuristic_cb_on_model(mdl)


if __name__ == "__main__":
    if len(sys.argv) < 2:
        data_file = join(dirname(abspath(__file__)), 'data', 'location.lp')
    elif len(sys.argv) == 2:
        ...
        print("             extension")
        sys.exit(-1)
    try_heuristic_cb_on_file(data_file)
